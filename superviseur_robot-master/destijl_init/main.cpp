...
void initStruct(void) {
    ...
    if (err = rt_sem_create(&sem_image, NULL, 0, S_FIFO)) {
        printf("Error semaphore create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&th_image, "th_image", 0, PRIORITY_TCAMERA, 0)) {
        printf("Error task create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
...
void startTasks() {
    ...
    if (err = rt_task_start(&th_image, &f_image, NULL)) {
        printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
